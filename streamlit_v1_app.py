...
def solve(df):
    ...
    routing.AddDimension(transit_callback_index, 0, 12 * 60, False, 'Time')
...
def run_with_timeout(df, timeout=60):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(solve, df)
        try:
            result, message = future.result(timeout=timeout)
            return result, message
        except concurrent.futures.TimeoutError:
            return None, "⏱ Temps limite dépassé : optimisation interrompue."
...
if uploaded_file is not None:
    try:
        ...
        if st.button("🚀 Lancer l'optimisation"):
            with st.spinner("Optimisation en cours (max 60 sec)..."):
                result_df, message = run_with_timeout(df)
                if result_df is not None:
                    ...
                    st.error(message or "Erreur inconnue.")
